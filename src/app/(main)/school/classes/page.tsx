...
export default function ClassesPage() {
  ...
  function toggleSection(name: string) {
    ...
    if (current.includes(name)) {
      classForm.setValue('sections', current.filter((s) => s !== name), { shouldValidate: true });
    } else {
      classForm.setValue('sections', [...current, name], { shouldValidate: true });
    }
  }
                  ...
                  <TableCell>
                    <Div type="row" gap="xs" wrap>
                      {clsSections.length === 0
                        ? <P size="xs">—</P>
                        : clsSections.map((s) => <Badge key={s.id} variant="info">{cls.name} — {s.name}</Badge>)
                      }
                    </Div>
                  </TableCell>
                  <TableCell>
            ...
            <ModalBody>
              <Div type="col" gap="md">
                ...
                <FormField label={CLASSES_PAGE.classForm.description}>
                  <Input placeholder="Optional description" {...classForm.register('description')} />
                </FormField>
                {/* Section checkboxes */}
                <FormField
