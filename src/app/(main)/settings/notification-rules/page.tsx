...
const ALL_EVENTS: NotificationEvent[] = [
  'ABSENT', 'LATE', 'HOLIDAY', 'LEAVE_APPROVED', 'LEAVE_REJECTED',
  'EARLY_EXIT', 'MISSING_PUNCH', 'GATE_PASS_APPROVED',
];
...
export default function NotificationRulesPage() {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button
            ...
            onClick={() => updateRule(row.original.event, { notify_parent: !rule.notify_parent })}
          >
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button
            ...
            loading={savingEvent === row.original.event}
            onClick={() => updateRule(row.original.event, { notify_student: !rule.notify_student })}
          >
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button
            ...
            loading={savingEvent === row.original.event}
            onClick={() => updateRule(row.original.event, { notify_teacher: !rule.notify_teacher })}
          >
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button
            ...
            loading={savingEvent === row.original.event}
            onClick={() => updateRule(row.original.event, { is_active: !rule.is_active })}
          >
  ...
  return (
    <PageCol>
      <PageHeader title="Notification Rules" subtitle="Configure when and how notifications are sent" />
