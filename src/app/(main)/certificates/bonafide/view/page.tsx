...
import { useBonafideCertificateDetail } from '@/hooks/useCertificates';
import { CERTIFICATE_PAGE, CERTIFICATE_VIEW } from '@/constants/certificate.constants';

function BonafideCertificateViewContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get('id') ?? undefined;
  const [showPdf, setShowPdf] = useState(false);

  const { certificate, isLoading } = useBonafideCertificateDetail(id);
      ...
      <Div type="grid" cols={2} gap="lg">
        ...
        <Div type="col" gap="sm" className="rounded-xl border border-border bg-card p-5">
          <H3 color="muted" className="uppercase tracking-wider text-xs font-semibold mb-2">
            {CERTIFICATE_VIEW.bonafide.sections.student}
          </H3>
          <InfoRow label={CERTIFICATE_VIEW.bonafide.labels.studentName} value={studentName} />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.dateOfBirth}
            value={
          ...
          />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.class}
            value={certificate.class?.name ?? '—'}
          />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.section}
            value={certificate.section?.name ?? '—'}
          />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.academicYear}
        ...
        <Div type="col" gap="sm" className="rounded-xl border border-border bg-card p-5">
          <H3 color="muted" className="uppercase tracking-wider text-xs font-semibold mb-2">
            {CERTIFICATE_VIEW.bonafide.sections.certificate}
          </H3>
          <InfoRow label={CERTIFICATE_VIEW.bonafide.labels.purpose} value={certificate.purpose} />
          <InfoRow label={CERTIFICATE_VIEW.bonafide.labels.referenceNo} value={certificate.reference_no} />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.status}
            value={CERTIFICATE_PAGE.status[certificate.status]}
          />
          <InfoRow
            label={CERTIFICATE_VIEW.bonafide.labels.issuedOn}
          ...
          <ModalBody>
            {certificate.pdf_url ? (
              <iframe
                ...
                style={{ height: '65vh' }}
                title={`Bonafide Certificate - ${studentName}`}
              />
            ) : (
...
export default function BonafideCertificateViewPage() {
  return (
    <Suspense
      fallback={
    ...
    >
      <BonafideCertificateViewContent />
    </Suspense>
  );
}