...
export function useAttendanceReports() {
  ...
  const [monthlyReport, setMonthlyReport] = useState<MonthlyAttendanceSummary[]>([]);
  ...
  async function fetchDailyReport() {
    if (!dailySectionId) { toast.error('Select a section'); return; }
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to fetch daily report');
    } finally {
  ...
  async function fetchMonthlyReport() {
    if (!monthlySectionId) { toast.error('Select a section'); return; }
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to fetch monthly report');
    } finally {
  ...
  async function fetchDefaulters() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to fetch defaulters');
    } finally {
  ...
  async function fetchStudentHistory(page = 1) {
    if (!historyStudentId) { toast.error('Enter a student ID'); return; }
    setIsLoadingHistory(true);
    try {
      const result = await AttendanceService.getStudentHistory(historyStudentId, { page, limit: 20 });
      setHistoryRecords(result.items);
      setHistoryPagination(result.pagination);
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : 'Failed to fetch student history');
    } finally {
  ...
  return {
    ...
    sections, isLoadingSections,

    // Daily
    dailySectionId, setDailySectionId,
    ...
    // Monthly
    monthlySectionId, setMonthlySectionId,
    ...
    // Defaulters
    defaulterSectionId, setDefaulterSectionId,
    defaulterThreshold, setDefaulterThreshold,
    defaulters,
    isLoadingDefaulters,
    fetchDefaulters,

    // Student history
    historyStudentId, setHistoryStudentId,
    historyRecords, historyPagination,
    isLoadingHistory,
    fetchStudentHistory,

    // Export
    exportSectionId, setExportSectionId,
